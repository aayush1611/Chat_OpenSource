...
- **Style isolation**: All Tailwind classes use the `fc-` prefix, and all styles are scoped under `.fynix-chat-root`. The widget never conflicts with host app styles.
...
```tsx
import { ChatProvider, ChatContainer, HistorySidebar } from '@fynix/chat-assistant';
